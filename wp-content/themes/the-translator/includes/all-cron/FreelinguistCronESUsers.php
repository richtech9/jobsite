...
class FreelinguistCronESUsers extends FreelinguistCronBase
{
    const OPTION_NAME = 'freelinguist_rebuild_elastic_users';
    const ACTION_NAME = 'freelinguist_rebuild_elastic_users';
    const STOP_ACTION_NAME = 'freelinguist_rebuild_elastic_users_command';
...
//                , $debug_string_command);
            ...
            $log[] = static::get_log_prefix() .' '.will_send_to_error_log(
                    "SQL with data completed for page $current_page --> $count_jobs Users found ",
                    '', $debug_string_command);
            ...
            if (count($jobs) > 0) {
                ...
                foreach ($jobs as $job) {
                    ...
                    $job_type = 'translator';
                    if ($job->wp_capabilities === FreelinguistUserLookupDataHelpers::USER_LOOKUP_CAPABILITIES_BUYER) {
                        $job_type = 'customer';
                    }

                    $params['body'][] = [
                        'job_id' => (int)$job->da_job_id,
                        'title' => $job->user_nicename,
                        'tags' => $tagArray,
                        'job_type' => $job_type,
                        'description' => $job->job_description,
                        'instruction' => '',
                        'is_cache' => '0',
                        'rating_as_freelancer' => (int)$job->rating_as_freelancer,
                        'rating_as_customer' => (int)$job->rating_as_customer,
                        'translate_from' => '',
                        'translate_to' => '',
                ...
                $log[] = static::get_log_prefix() .' '.will_send_to_error_log(
                        "No Users in current page $current_page ,nothing to do, and not rescheduling action ",
                        FREELINGUIST_WILL_LOG_NO_VALUE, $debug_string_command);
            }
...
FreelinguistCronESUsers::set_up_hook();