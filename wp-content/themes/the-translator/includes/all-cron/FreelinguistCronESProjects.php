...
class FreelinguistCronESProjects extends FreelinguistCronBase
{
    const OPTION_NAME = 'freelinguist_rebuild_elastic_projects';
    const ACTION_NAME = 'freelinguist_rebuild_elastic_projects';
    const STOP_ACTION_NAME = 'freelinguist_rebuild_elastic_projects_command';
    ...
    public static function main($extra_command)
    {
        ...
        try {
            ...
            $log = self::get_log();
            $log[] = static::get_log_prefix() .' '.will_send_to_error_log("Starting partial cron job $index! v1.1",
            ...
            WHERE
              ...
              look.fl_job_type = " . FLPostLookupDataHelpers::POST_DATA_JOB_TYPE_PROJECT . "
              ;
    ";
...
//                , $debug_string_command);
            ...
            if ($current_page >= $total_job_pages) {
                ...
                WHERE
                  wppost.post_type = 'job' and
                  wppost.post_status = 'publish' and
                  look.fl_job_type = " . FLPostLookupDataHelpers::POST_DATA_JOB_TYPE_PROJECT . " and
                  look.hide_job = 0
                 ORDER BY wppost.ID  
                  $limit_part
                  ";
                ...
                $log[] = static::get_log_prefix() .' '.will_send_to_error_log(
                    "SQL with data completed for page $current_page --> $count_jobs Projects found ",
                    '', $debug_string_command);
...
FreelinguistCronESProjects::set_up_hook();