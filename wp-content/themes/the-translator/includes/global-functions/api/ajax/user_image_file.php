 ...
 * Method:      user_image_file

 * Description: user_image_file

 *

 */

add_action('wp_ajax_user_image_file', 'user_image_file');


function user_image_file()
{

    /*
    * current-php-code 2020-Oct-17
    * ajax-endpoint  user_image_file
    * input-sanitized :
    */
    ...
    try {
        ...
        if ($_FILES['files']['size'] > $max_file_size) {
            ...
            if (move_uploaded_file($_FILES["files"]["tmp_name"], $original_file_path)) {
                try {
        ...
        will_send_to_error_log('user_image_file', will_get_exception_string($e));
