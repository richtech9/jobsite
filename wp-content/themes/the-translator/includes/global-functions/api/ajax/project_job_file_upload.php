 ...
 * Method:      project_job_file_upload
...
add_action('wp_ajax_project_job_file_upload', 'project_job_file_upload');

/**
 * code-notes Only when the user is the project owner
 */
function project_job_file_upload(){
    global $wpdb;
    /*
        * current-php-code 2020-Sep-30
        * ajax-endpoint  project_job_file_upload
        * input-sanitized : project_id
        *  when when project owner is uploading instruction files on the job details screen
        */
    ...
    try {
        ...
        will_throw_on_wpdb_error($wpdb, 'project_job_file_upload check permissions');
        ...
        if (!empty($response['files'])) {
            ...
            foreach ($response['files'] as $f) {
                ...
                if (!isset($f->error)) {
                    ...
                    $our_file_name = 'project-' . $project_id . '-' . time() . '-' . UploadHandler::generate_random_safe_characters();
                    ...
                    if (!$b_rename_ok) {
                        ...
                        throw new RuntimeException("project_job_file_upload failed to rename $old_full_path to $our_full_path ");
                    }
                    ...
                    will_throw_on_wpdb_error($wpdb, 'project_job_file_upload inserting file project file upload');
        ...
        will_send_to_error_log('project_job_file_upload has error',will_get_exception_string($e));
