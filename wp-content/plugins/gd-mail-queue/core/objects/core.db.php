...
class gdmaq_core_db extends d4p_wpdb_core {
    ...
    public function add_mail_to_queue($args = array()) {
        if (!isset($args['blog_id']) || empty($args['blog_id']) || $args['blog_id'] === false || $args['blog_id'] == 0) {
    ...
    public function email_log_cleanup($blog_id = 0, $days = 365) {
        $sql = "DELETE l, e FROM ".$this->log." l INNER JOIN ".$this->log_email." e ON e.`log_id` = l.`id` WHERE l.`logged` < DATE_SUB(CURDATE(), INTERVAL ".absint($days)." DAY)";
    ...
    public function queue_cleanup($blog_id = 0, $scope = array(), $days = 30) {
        $sql = "DELETE FROM ".$this->queue." WHERE `status` IN ('".join("', '", $scope)."') AND `queued` < DATE_SUB(CURDATE(), INTERVAL ".absint($days)." DAY)";

        $this->query($sql);
    }

    public function queue_counts($blog_id = 0) {
        $sql = "SELECT `status`, COUNT(*) AS items FROM ".$this->queue." GROUP BY `status`";
