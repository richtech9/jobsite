...
class gdmaq_core_htmlfy {
    ...
    public function __construct() {
        foreach (array('replace', 'template', 'additional', 'header', 'footer', 'preheader', 'preheader_limit', 'embed_local_images') as $key) {
    ...
    public function init() {
        $this->_poweredby = apply_filters('gdmaq_htmlfy_poweredby',
            '<a href="https://plugins.dev4press.com/gd-mail-queue/">GD Mail Queue plugin for WordPress by Dev4Press</a>');
    ...
    public function htmlfy_content($content, $subject, $args = array()) {
        ...
        $html_content = $args['is_html'] ? $content : $this->_process_text_to_html($content);
        ...
        $tags = array(
            ...
            '{{EMAIL_POWEREDBY}}' => $this->_poweredby,
    ...
    private function _convert_textlinks($body) {
        return preg_replace('#<(https?://[^*]+)>#', '$1', $body);
    }

    private function _process_text_to_html($body) {
        $body = $this->_convert_textlinks($body);
        $body = nl2br($body);
