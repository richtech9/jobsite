...
class gdmaq_admin_settings {
        ...
        $settings = apply_filters('gdmaq_admin_internal_settings', array(
            ...
            'queue' => array(
                ...
                'classic' => array('name' => __("Classic Queue Processing", "gd-mail-queue"), 'settings' => array(
                    new d4pSettingElement('queue', 'timeout', __("Maximum Job Length", "gd-mail-queue"), __("Limit the length of each Cron Job. This should typically be lower than the PHP script timeout on your server.", "gd-mail-queue"), d4pSettingType::ABSINT, gdmaq_settings()->get('timeout', 'queue'), '', '', array('label_unit' => __("Seconds", "gd-mail-queue"))),
            ...
            'htmlfy' => array(
                'template' => array('name' => __("HTML Template", "gd-mail-queue"), 'settings' => array(
                ...
                'template_template' => array('name' => __("Built-in HTML Templates", "gd-mail-queue"), 'settings' => array(
    ...
    private function get_phpmailer_mode() {
        return array(
    ...
    private function get_phpmailer_encryption() {
        return array(
    ...
    private function get_queue_methods() {
        return array(
    ...
    private function get_htmlfy_replace() {
        $list = array(
    ...
    private function get_htmlfy_plain_text_detection() {
        return array(
    ...
    private function get_htmlfy_additional_templates() {
        return gdmaq()->get_additional_templates_list();
    }

    private function get_htmlfy_templates() {
        return array(
    ...
    private function get_htmlfy_preheader() {
        return array(
    ...
    public function get_queue_method() {
        return array(
            'classic' => __("Classic", "gd-mail-queue")
        );
    }

    private function get_shared_template_tags() {
        return gdmaq_admin()->get_shared_template_tags();
    }

    private function get_template_tags() {
        return array_merge(array(
            '{{EMAIL_SUBJECT}}' => __("Subject", "gd-mail-queue").' <strong>('.__("Required", "gd-mail-queue").')',
            '{{EMAIL_CONTENT}}' => __("Content", "gd-mail-queue").' <strong>('.__("Required", "gd-mail-queue").')',
            '{{EMAIL_PREHEADER}}' => __("Preheader", "gd-mail-queue"),
            '{{EMAIL_HEADER}}' => __("Header", "gd-mail-queue"),
            '{{EMAIL_FOOTER}}' => __("Footer", "gd-mail-queue"),
            '{{EMAIL_POWEREDBY}}' => __("Powered By", "gd-mail-queue")
        ), $this->get_shared_template_tags());
    }

    public function get_cleanup_scope() {
        return array(
    ...
    private function render_toggle_table($title, $elements) {
