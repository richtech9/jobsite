...
class gdmaq_admin_ajax {
    ...
    public function emailtest() {
        ...
        $test_settings.= __("PHPMailer Version", "gd-mail-queue").': <strong>'.$phpmailer->Version.'</strong><br/>';

        foreach ($props as $property) {
            $test_settings.= $property.': <strong>'.$phpmailer->$property.'</strong><br/>';
        }
        //code-bookmark Added more php mailer data to test screen. This is where the mail queue test is at
        $test_settings .= "extra info for phpmailer<br>";
        foreach ($phpmailer as $property => $value) {
            if (is_object($value) || is_array($value)) {
                $value = json_encode($value);
            }
            $test_settings.= $property.': <strong>'.$value.'</strong><br/>';
        }
        ...
        if ($phpmailer->ErrorInfo != '') {
            ...
            $test_message = __("An error occured", "gd-mail-queue").':<br/><strong>'.$phpmailer->ErrorInfo.'</strong>';
        }
