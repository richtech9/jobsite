...
class gdmaq_emails_log extends d4p_grid {
    ...
    private function _render_tag($tag, $label, $title = '') {
        $render = '<span class="gdmaq-item-tag gdmaq-tag-'.$tag.'"'.(!empty($title) ? ' title="'.$title.'"' : '').'>'.$label.'</span>';

        return $render;
    }

    private function _render_email($email, $rel) {
        $render = '<span class="gdmaq-single-email gdmaq-email-'.$rel.'">'.$email->email.'</span>';
    ...
    public function get_views() {
        ...
        return array(
            ...
            'mail' => '<a href="'.add_query_arg('operation', 'mail', $url).'" class="'.($this->_operation == 'mail' ? 'current' : '').'">'.__("WP Mail", "gd-mail-queue").'</a>',
            'queue' => '<a href="'.add_query_arg('operation', 'queue', $url).'" class="'.($this->_operation == 'queue' ? 'current' : '').'">'.__("Queue", "gd-mail-queue").'</a>'
        );
    }
    ...
    public function get_row_classes($item) {
        $classes = array('gdmaq-log-row-'.$item->status);

        return $classes;
    }

    public function rows_per_page() {
        $per_page = get_user_option('gdmaq_rows_per_page_log');
    ...
    public function column_basic($item) {
        $render = '';
    ...
    public function column_status($item) {
        ...
        if ($item->status == 'ok') {
            ...
            $render.= $this->_render_tag('fail', __("Sending Failed", "gd-mail-queue"), $item->message);
    ...
    public function column_logged($item) {
    ...
    public function prepare_items() {
        ...
        $status = isset($_GET['filter-status']) && !empty($_GET['filter-status']) ? d4p_sanitize_slug($_GET['filter-status']) : '';
        $search = isset($_GET['s']) && $_GET['s'] != '' ? d4p_sanitize_basic($_GET['s']) : '';
        $type = isset($_GET['filter-type']) && !empty($_GET['filter-type']) ? d4p_sanitize_slug($_GET['filter-type']) : '';
