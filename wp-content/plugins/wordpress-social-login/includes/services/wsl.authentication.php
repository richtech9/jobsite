...
function wsl_process_login_create_wp_user( $provider, $hybridauth_user_profile, $requested_user_login, $requested_user_email )
{
	...
	if(!empty(getSocialUserType())){
		$userdata['role'] = getSocialUserType();
	}
	...
	if( ! $user_id || ! is_integer( $user_id ) )
	{
		...
		$user_residence_country = get_user_meta( $user_id, 'user_residence_country', true);
        if(empty($user_residence_country)){
            update_user_meta( $user_id, 'user_residence_country', '224');
        }
	}
